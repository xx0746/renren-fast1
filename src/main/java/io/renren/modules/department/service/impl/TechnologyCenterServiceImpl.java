...
import io.renren.modules.department.mapper.TechnologyCenterMapper;
 ...
 */
@Service
public class TechnologyCenterServiceImpl extends ServiceImpl<TechnologyCenterMapper, TechnologyCenter> implements TechnologyCenterService {

    @Override
    public Page<TechnologyCenter> pageListWithCondition(CommentVO commentVO, Integer current, Integer pageSize) {
        Page<TechnologyCenter> page = new Page<>(current,pageSize);
        //构建查询条件
    ...
    public void exportExcel(CommentVO commentVO, HttpServletResponse response) {
        ...
        wrapper.like(StringUtils.isNotEmpty(commentVO.getName()),"name",commentVO.getName())
                .like(StringUtils.isNotEmpty(commentVO.getCreateTime()),"create_time",commentVO.getCreateTime())
                .orderByAsc("sortId");
        List<TechnologyCenter> technologyList = baseMapper.selectList(wrapper);
        //通过查询出来的数据写到浏览器
        ...
        //向浏览器写入数据
        try {
            EasyExcel.write(response.getOutputStream(), TechnologyCenter.class).sheet("模板").doWrite(technologyList);
        }catch (IOException e){
            e.printStackTrace();
        }
    }
