...
import io.renren.modules.department.mapper.IntelligenceCenterMapper;
 ...
 */
@Service
public class IntelligenceCenterServiceImpl extends ServiceImpl<IntelligenceCenterMapper, IntelligenceCenter> implements IntelligenceCenterService {

    @Override
    public Page<IntelligenceCenter> pageListWithCondition(CommentVO commentVO, Integer current, Integer pageSize) {
        Page<IntelligenceCenter> page = new Page<>(current,pageSize);
        //构建查询条件
        ...
        wrapper.like(StringUtils.isNotEmpty(commentVO.getName()),"name",commentVO.getName())
                ...
                .orderByAsc("sortId+0");
        Page<IntelligenceCenter> intelligencePage = baseMapper.selectPage(page, wrapper);
        intelligencePage.getRecords().sort((o1, o2) -> {
    ...
    public void exportExcel(CommentVO commentVO, HttpServletResponse response) {
        ...
        //向浏览器写入数据
        try {
            EasyExcel.write(response.getOutputStream(),IntelligenceCenter.class).sheet("模板").doWrite(intelligenceList);
        }catch (IOException e){
            e.printStackTrace();
        }
    }
